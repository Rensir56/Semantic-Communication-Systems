...
from model_usps_mnist import G12, G21
from model_usps_mnist import D1, D2
import scipy.misc
import imageio
...
class Solver(object):
    ...
    def merge_images_encoder(self, sources, targets, decoder, k=10):
        _, _, h, w = sources.shape
        row = int(np.sqrt(self.batch_size))
        merged = np.zeros([3, row * h, row * w * 3])  # 三维，然后列扩展成了3倍
    ...
    def train(self):
        ...
        for iii in range(9):
            ...
            # fixed mnist and usps for sampling
            # print('usps_iter.next()[0]', usps_iter.next()[0].size())
            # print('mnist_iter.next()[0]', mnist_iter.next()[0].size())
            fixed_usps = self.to_var(usps_iter.next()[0])
            ...
            for step in range(self.train_iters + 1):
                ...
                # load usps and mnist dataset
                usps, s_labels = usps_iter.next()
                usps, s_labels = self.to_var(usps), self.to_var(s_labels).long().squeeze()
                mnist, m_labels = mnist_iter.next()
                mnist, m_labels = self.to_var(mnist), self.to_var(m_labels)

                if self.use_labels:
                    mnist_fake_labels = self.to_var(
                        torch.Tensor([self.num_classes] * usps.size(0)).long())
                    usps_fake_labels = self.to_var(
                ...
                self.reset_grad()
                fake_usps = self.g12(mnist)
                out = self.d2(fake_usps)
                if self.use_labels:
                    d2_loss = criterion(out, usps_fake_labels)
                else:
                    d2_loss = torch.mean(out ** 2)

                fake_mnist = self.g21(usps)
                ...
                # train mnist-usps-mnist cycle
                self.reset_grad()
                fake_usps = self.g12(mnist)
                out = self.d2(fake_usps)
                reconst_mnist = self.g21(fake_usps)
                if self.use_labels:
                ...
                # train usps-mnist-usps cycle
                self.reset_grad()
                fake_mnist = self.g21(usps)
                out = self.d1(fake_mnist)
                reconst_usps = self.g12(fake_mnist)

                xx = fake_mnist.detach().cpu().numpy()
                xxx = int(np.size(xx))
                if xxx != 50176:
                    # print('There is an error in shape of data, the true batch size:', xxx)
                    continue
                fake_mnist_reshape = fake_mnist.view(64, -1)  # 64,1,28,28转64,784
                ...
                if self.use_reconst_loss:
                    g_loss += torch.mean((usps - reconst_usps) ** 2)
                ...
                if (step + 1) % self.log_step == 0:
                    # print('Step [%d/%d], d_real_loss: %.4f, d_mnist_loss: %.4f, d_usps_loss: %.4f, '
                    #       'd_fake_loss: %.4f, g_loss: %.4f'
                    #       %(step+1, self.train_iters, d_real_loss.data[0], d_mnist_loss.data[0],
                    #         d_usps_loss.data[0], d_fake_loss.data[0], g_loss.data[0]))
                    # print(d_real_loss.size())
                    print('Step [%d/%d], acc: %.4f, d_real_loss: %.6f, d_mnist_loss: %.6f, d_usps_loss: %.6f, '
                          'd_fake_loss: %.6f, g_loss: %.4f'
                          % (step + 1, self.train_iters, acc, d_real_loss.item(), d_mnist_loss.item(),
                             d_usps_loss.item(), d_fake_loss.item(), g_loss.item()))

                # save the sampled images
                if (step + 1) % self.sample_step == 0:
                    # 保存图，需要的时候再打开

                    # fake_usps = self.g12(fixed_mnist)
                    # fake_mnist = self.g21(fixed_usps)
                    #
                    ...
                    # usps, fake_usps = self.to_data(fixed_usps), self.to_data(fake_usps)
                    #
                    # out_encoder_reshape_data = self.to_data(out_encoder_reshape)
                    #
                    # # mnist生成usps的结果
                    # merged = self.merge_images(mnist, fake_usps)
                    ...
                    #
                    # # usps生成mnist的结果
                    # merged = self.merge_images(usps, fake_mnist)
                    ...
                    # # print('range of fake:', np.max(fake_mnist), np.min(fake_mnist), np.shape(fake_mnist))
                    # # print('range of out:', np.max(out_encoder_reshape_data), np.min(out_encoder_reshape_data), np.shape(out_encoder_reshape_data))
                    # merged = self.merge_images_encoder(usps, fake_mnist, out_encoder_reshape_data)
                    ...
                    file = ('./results/acc_usps_mnist_%.2f.csv' % compression_rate)
